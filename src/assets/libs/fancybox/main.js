(function ($) {
	...
	$('.item_href').on('click', function (e) {
		e.preventDefault();
	})



	let images = document.querySelectorAll(".portfolio_nav-item");
	let current = 0;
	...
	function portfolioToggle() {
		...
		let imgToggleCurrent = images[current].getAttribute('data-toggle')

		$("[data-main]").each(function () {
			let imgMain = $(this).data('main');
			if (imgMain == imgToggleCurrent) {
	...
	$('.product-view-mode a').on('click', function (e) {
		...
		shopProductWrap.removeClass('grid-view-1 grid-view-2 grid-view-3 grid-view-4 grid-view-5').addClass(viewMode);
	...
	if (Wwidth <= 991) {
		// blog carousel active
		$('.potfolio_main-wrapper').addClass('slick-arrow-style')
		$('.portfolio_nav').slick({
			autoplay: false,
			focusOnSelect: true,
			speed: 1000,
			arrows: false,
			slidesToShow: 6,
			asNavFor: '.potfolio_main-wrapper',
		...
		$('.potfolio_main-wrapper').slick({
			arrows: true,
			prevArrow: '<button type="button" class="slick-prev"><i class="pe-7s-angle-left"></i></button>',
			nextArrow: '<button type="button" class="slick-next"><i class="pe-7s-angle-right"></i></button>',
			asNavFor: '.portfolio_nav'
		});
	} else {
		// setInterval(portfolioToggle, 5000);
		let toggle = $('[data-toggle]');

		$(".portfolio_nav-item").on("click", function () {
			$(this).addClass("current").siblings().removeClass("current")
			let cat = $(this).data('toggle');
			$("[data-main]").each(function () {
				let imgMain = $(this).data('main');
				if (imgMain == cat) {
					$(this).addClass('current');
